<template>
  ...
  <div class="w-97 mx-auto" :class="{ 'overflow-hidden' : showModal || showPostModal || showReviewModal || showFollowModal}">
    <FollowListModal v-if="showFollowModal" @close="showFollowModal = false" id="modal">
      <!-- 1-1) 모달 헤더 -->
      <h3 slot="header" class="bc-card text-base flex items-center">{{title}}</h3> 
      <!-- 1-2) 모달 푸터 -->
      <span slot="body">
          <ul>
  ...
  methods: {
    ...
    onClick(value){
      ...
      if (value==='like') {
        ...
        this.title = `${this.profile.username}님의 찜목록`
      } else{
        this.contents = this.profile.looked
        this.title = `${this.profile.username}님이 본 영화`
      }
    },
    onPostClick(){
      this.showPostModal = !this.showPostModal
        this.contents = this.profile.articles
        this.title = `${this.profile.username}님의 최근 게시글`
    },
    onReviewClick(){
      this.showReviewModal = !this.showReviewModal
        this.contents = this.profile.reviews_list
        this.title = `${this.profile.username}님의 최근 리뷰`
    },
    onCheckFollow(value){
      this.showFollowModal = !this.showFollowModal
      if (value ==='followings') {
        this.contents = this.profile.followings
        this.title = `${this.profile.username}'s Following List`
      } else{
        this.contents = this.profile.followers
        this.titlel = `${this.profile.username}'s Follower List`
      }
    }
  },
  created() {
